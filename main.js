...
class VaultRadioPlugin extends Plugin {
    ...
    setupUI() {
        ...
        this.updateFavButton(); this.updateModeIcon();
    ...
    async loadFullPlaylist() { this.fullPlaylist = []; if (this.settings.musicFolderPaths.length > 0) { const allFiles = this.app.vault.getFiles(); const filesByFolder = await Promise.all(this.settings.musicFolderPaths.map(async (folderPath) => { if (!folderPath) return []; const normalizedPath = folderPath.replace(/\\/g, '/'); return allFiles.filter(f => f.path.startsWith(normalizedPath) && ['flac', 'mp3', 'wav', 'm4a', 'ogg'].includes(f.extension.toLowerCase())); })); this.fullPlaylist = filesByFolder.flat().map((f, index) => ({ id: index, name: f.basename, path: f.path, resourcePath: this.app.vault.getResourcePath(f) })); } this.updateCategorySelector(); this.updateView(); }
    ...
    renderPlaylist() { this.hubPlaylist.empty(); if (this.viewPlaylist.length === 0) { this.hubPlaylist.createEl('li', { text: "此列表为空" }); return; } this.viewPlaylist.forEach((track) => { const li = this.hubPlaylist.createEl('li', { text: track.name }); if (this.nowPlayingTrack && track.id === this.nowPlayingTrack.id) li.addClass('is-playing'); li.draggable = true; li.addEventListener('dragstart', (ev) => ev.dataTransfer.setData("text/plain", `![[${track.path}]]`)); li.addEventListener('click', () => { this.loadTrack(track, true); }); this.hubPlaylist.appendChild(li); }); }
    ...
    updateCategorySelector() { this.categorySelect.empty(); this.categorySelect.add(new Option("所有歌曲", "all")); this.categorySelect.add(new Option("喜爱列表", "favorite")); if (this.settings.musicFolderPaths.length > 1) { this.settings.musicFolderPaths.forEach(path => { if (path) this.categorySelect.add(new Option(path.split('/').pop() || path, path)); }); } }
    ...
    handleFileChange(path) { if (this.settings.musicFolderPaths.some(p => path.startsWith(p))) { setTimeout(() => this.loadFullPlaylist(), 500); } }
