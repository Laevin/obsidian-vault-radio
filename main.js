...
class VaultRadioPlugin extends Plugin {
    ...
    async loadFullPlaylist() {
        this.fullPlaylist = [];
        const validFolders = this.settings.musicFolderPaths.filter(p => p && p.trim() !== '');
        if (validFolders.length > 0) {
            ...
            validFolders.forEach(folderPath => {
                ...
                allFiles.forEach(file => {
                    if (file.path.startsWith(normalizedPath) && ['flac', 'mp3', 'wav', 'm4a', 'ogg'].includes(file.extension.toLowerCase())) {
                        collectedFiles.set(file.path, file);
                    }
                });
            });
            
            this.fullPlaylist = Array.from(collectedFiles.values()).map((f, index) => ({ 
                id: index, name: f.basename, path: f.path, resourcePath: this.app.vault.getResourcePath(f) 
            }));
        }
    ...
    updateCategorySelector() {
        ...
        this.categorySelect.add(new Option("所有歌曲", "all"));
        this.categorySelect.add(new Option("喜爱列表", "favorite"));

        const validFolders = this.settings.musicFolderPaths.filter(p => p && p.trim() !== '');
        validFolders.forEach(path => {
            this.categorySelect.add(new Option(path.split('/').pop() || path, path));
        });
        
        if (Array.from(this.categorySelect.options).some(option => option.value === currentVal)) {
    ...
    setupUI() {
        ...
        this.statusBarItem.addClass('minimal-player-statusbar-new');
        this.prevButton = this.statusBarItem.createEl('button', { cls: 'minimal-player-button' }); setIcon(this.prevButton, 'skip-back');
        this.playPauseButton = this.statusBarItem.createEl('button', { cls: 'minimal-player-button' }); setIcon(this.playPauseButton, 'play');
        this.nextButton = this.statusBarItem.createEl('button', { cls: 'minimal-player-button' }); setIcon(this.nextButton, 'skip-forward');
        this.playlistTrackButton = this.statusBarItem.createEl('button', { cls: 'playlist-track-button' });
        this.trackNameEl = this.playlistTrackButton.createEl('span', { text: '播放列表' });
        this.statusBarProgress = this.playlistTrackButton.createEl('div', { cls: 'status-bar-progress' });
        this.hubContainer = document.body.createEl('div', { cls: 'minimal-player-hub-container' });
        this.hubContainer.hide();
        const hubFunctionBar = this.hubContainer.createEl('div', { cls: 'hub-function-bar' });
        this.favButton = hubFunctionBar.createEl('button', { cls: 'hub-function-button' });
        this.categorySelect = hubFunctionBar.createEl('select', { cls: 'hub-function-select' });
        this.modeButton = hubFunctionBar.createEl('button', { cls: 'hub-function-button' });
        this.updateFavButton();
        this.updateModeIcon();
        const hubControls = this.hubContainer.createEl('div', { cls: 'hub-controls' });
        this.hubProgressContainer = hubControls.createEl('div', { cls: 'hub-progress-container' });
        this.hubProgressFill = this.hubProgressContainer.createEl('div', { cls: 'hub-progress-fill' });
        this.hubProgressThumb = this.hubProgressContainer.createEl('div', { cls: 'hub-progress-thumb' });
        this.hubTimeDisplay = hubControls.createEl('div', { cls: 'hub-time-display' });
        this.hubPlaylist = this.hubContainer.createEl('ul', { cls: 'hub-playlist' });
    }
    ...
    registerEventListeners() {
        ...
        this.registerDomEvent(this.nextButton, 'click', () => this.playNext());
        this.registerDomEvent(this.playlistTrackButton, 'click', () => this.toggleHub());
        this.registerDomEvent(this.audioPlayer, 'ended', () => this.playNext());
    ...
    updateProgress() {
        ...
        if (isNaN(duration) || !isFinite(duration)) {
            this.hubTimeDisplay.setText("--:-- / --:--");
            this.statusBarProgress.style.width = '0%';
            this.hubProgressFill.style.width = '0%';
            this.hubProgressThumb.style.left = '-5px';
            return;
        }
        const p = (currentTime / duration) * 100;
        this.statusBarProgress.style.width = `${p}%`;
        this.hubProgressFill.style.width = `${p}%`;
        this.hubProgressThumb.style.left = `calc(${p}% - 5px)`;
        const f = (s) => new Date(s * 1000).toISOString().slice(14, 19);
        this.hubTimeDisplay.setText(`${f(currentTime)} / ${f(duration)}`);
    }
    ...
    updateView() {
        ...
        if (category === 'all') { sourcePlaylist = this.fullPlaylist; } 
        else if (category === 'favorite') { sourcePlaylist = this.fullPlaylist.filter(t => this.settings.favorites.includes(t.path)); } 
        else { sourcePlaylist = this.fullPlaylist.filter(t => t.path.startsWith(category)); }
        
        if (this.settings.playbackMode === 'shuffle') { this.viewPlaylist = [...sourcePlaylist].sort(() => Math.random() - 0.5); } 
        else { this.viewPlaylist = sourcePlaylist; }
        
        if (!this.nowPlayingTrack || !this.fullPlaylist.some(t => t.path === this.nowPlayingTrack.path)) {
    ...
    loadTrack(track, autoPlay) {
        if (!track) {
            ...
            this.trackNameEl.setText(track.name);
            if (this.audioPlayer.src !== track.resourcePath) {
                this.audioPlayer.src = track.resourcePath;
            }
        }
    ...
    playNext() { if (this.viewPlaylist.length === 0) return; if (this.settings.playbackMode === 'single' && this.nowPlayingTrack) { this.audioPlayer.currentTime = 0; this.play(); return; } const currentIndexInView = this.nowPlayingTrack ? this.viewPlaylist.findIndex(t => t.id === this.nowPlayingTrack.id) : -1; const nextIndex = (currentIndexInView + 1) % this.viewPlaylist.length; this.loadTrack(this.viewPlaylist[nextIndex], true); }
    playPrevious() { if (this.viewPlaylist.length === 0) return; const currentIndexInView = this.nowPlayingTrack ? this.viewPlaylist.findIndex(t => t.id === this.nowPlayingTrack.id) : -1; const prevIndex = (currentIndexInView - 1 + this.viewPlaylist.length) % this.viewPlaylist.length; this.loadTrack(this.viewPlaylist[prevIndex], true); }
    
    renderPlaylist() {
        this.hubPlaylist.empty();
        if (this.viewPlaylist.length === 0) { this.hubPlaylist.createEl('li', { text: "此列表为空" }); return; }
        this.viewPlaylist.forEach((track) => {
            const li = this.hubPlaylist.createEl('li', { text: track.name });
            if (this.nowPlayingTrack && track.id === this.nowPlayingTrack.id) li.addClass('is-playing');
            li.draggable = true;
            li.addEventListener('dragstart', (ev) => ev.dataTransfer.setData("text/plain", `![[${track.path}]]`));
            li.addEventListener('click', () => { this.loadTrack(track, true); });
            this.hubPlaylist.appendChild(li);
        });
    }
    ...
    display() {
            ...
            new Setting(containerEl)
                .addText(text => text
                    ...
                    .onChange(async (value) => {
                        this.plugin.settings.musicFolderPaths[index] = value.trim().replace(/\\/g, '/');
                        await this.plugin.saveSettings();
                        this.plugin.loadFullPlaylist();
                    }))
                .addExtraButton(button => button
                    .setIcon('trash')
                    .setTooltip('删除')
                    .onClick(async () => {
                        this.plugin.settings.musicFolderPaths.splice(index, 1);
                        await this.plugin.saveSettings();
                        this.plugin.loadFullPlaylist();
                        this.display();
                    }));
        ...
        new Setting(containerEl)
            .addButton(button => button
                ...
                .onClick(async () => {
                    this.plugin.settings.musicFolderPaths.push("");
                    await this.plugin.saveSettings();
                    this.display();
                }));
    }
