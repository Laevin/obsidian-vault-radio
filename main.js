...
class VaultRadioPlugin extends Plugin {
    ...
    // --- 核心修复 1：重写 loadFullPlaylist 以进行去重 ---
        ...
        if (validFolders.length > 0) {
            ...
            validFolders.forEach(folderPath => {
                ...
                const filesInFolder = allFiles.filter(f => 
                    ...
                    ['flac', 'mp3', 'wav', 'm4a', 'ogg'].includes(f.extension.toLowerCase())
                );
                collectedFiles.push(...filesInFolder);
            });
            
            const uniqueFiles = new Map(collectedFiles.map(f => [f.path, f]));
            this.fullPlaylist = Array.from(uniqueFiles.values()).map((f, index) => ({ 
    ...
    // --- 核心修复 2：重写 updateCategorySelector 以正确处理显示 ---
    ...
    registerEventListeners() { this.registerDomEvent(this.playPauseButton, 'click', () => this.togglePlayPause()); this.registerDomEvent(this.prevButton, 'click', () => this.playPrevious()); this.registerDomEvent(this.nextButton, 'click', () => this.playNext()); this.registerDomEvent(this.playlistTrackButton, 'click', () => this.toggleHub()); this.registerDomEvent(this.audioPlayer, 'ended', () => this.playNext()); this.registerDomEvent(this.audioPlayer, 'timeupdate', () => this.updateProgress()); this.registerDomEvent(this.hubProgressContainer, 'mousedown', this.handleDragStart); this.registerDomEvent(this.modeButton, 'click', () => this.togglePlaybackMode()); this.registerDomEvent(this.favButton, 'click', () => this.toggleFavorite()); this.registerDomEvent(this.categorySelect, 'change', () => this.handleCategoryChange()); this.registerEvent(this.app.vault.on('create', (file) => this.handleFileChange(file.path))); this.registerEvent(this.app.vault.on('delete', (file) => this.handleFileChange(file.path))); this.registerEvent(this.app.vault.on('rename', (file, oldPath) => this.handleFileChange(oldPath))); }
